...
function getExName(log, exKey) {
  ...
  return session.exercises[idx]?.name || null;
}
...
function getExerciseHistory(logs, exName) {
  ...
  logs.forEach(([, log]) => {
    ...
    points.push({ date: log.date, maxWeight, totalVolume: Math.round(totalVol), avgReps });
  });
...
function getSessionDurations(logs) {
  return logs
    ...
    .map(([, log]) => {
      ...
      const session = ALL_SESSIONS[log.regime]?.[log.sessionId];
      return { date: log.date, duration: mins, label: session?.label || log.sessionId };
    })
    .filter(Boolean)
    .sort((a, b) => a.date.localeCompare(b.date));
}
...
const SL = ({ children, style = {} }) => (
  <div style={{ fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: "#888", margin: "28px 0 12px", ...style }}>
    {children}
  </div>
);

const Card = ({ children, style = {} }) => (
  <div style={{ background: "#fff", border: "1px solid #e5e5e5", padding: "16px", marginBottom: 12, ...style }}>
    {children}
  </div>
);
...
const ttStyle = {
  background: "#fff", border: "1px solid #e5e5e5", borderRadius: 3,
  fontSize: 12, fontFamily: "Georgia, serif", padding: "8px 12px", color: "#111",
};
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "flex-start" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 3, paddingTop: 1 }}>
                {["M","T","W","T","F","S","S"].map((d, i) => (
                  <div key={i} style={{ width: 10, height: 14, fontSize: 9, color: "#ccc", display: "flex", alignItems: "center" }}>
                    {d}
                  </div>
                ))}
              </div>
      ...
      {activeSection === "progression" && (
        <div>
          ...
          <Card>
            ...
            {!selectedExercise && (
              <EmptyState text="Pick an exercise to see your weight progression over time." />
            )}
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={exerciseHistory} margin={{ top: 5, right: 16, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip contentStyle={ttStyle} formatter={v => [`${v}kg`, "Max weight"]} labelFormatter={shortFmt} />
                    {exerciseHistory.length > 1 && (
                      <ReferenceLine
                        y={Math.max(...exerciseHistory.map(d => d.maxWeight))}
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={volumeHistory} margin={{ top: 5, right: 16, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip contentStyle={ttStyle} formatter={v => [`${v.toLocaleString()}kg`, "Volume"]} labelFormatter={shortFmt} />
      ...
      {activeSection === "personal bests" && (
        <div>
          ...
          {personalBests.length === 0 ? (
            <Card>
              <EmptyState text="No personal bests yet." />
            </Card>
          ) : (
              ...
              {personalBests.map(([name, pb], i) => (
                <div key={name} style={{ padding: "10px 14px", borderBottom: i < personalBests.length - 1 ? "1px solid #f9f9f9" : "none", display: "grid", gridTemplateColumns: "1fr 60px 60px 80px", gap: 8, alignItems: "center" }}>
                  <span style={{ fontSize: 13, fontWeight: i === 0 ? 700 : 400, color: "#111" }}>
                    <span style={{ fontSize: 10, background: i === 0 ? "#fff8e1" : "#f5f5f5", color: i === 0 ? "#b45309" : "#aaa", padding: "1px 5px", borderRadius: 2, fontWeight: 700, marginRight: 6 }}>
                      PB
                    </span>
                    {name}
                  </span>
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={Math.max(200, avgRepsData.slice(0, 12).length * 32)}>
                  <BarChart data={avgRepsData.slice(0, 12)} layout="vertical" margin={{ top: 0, right: 40, left: 0, bottom: 0 }}>
                    ...
                    />
                    <Tooltip contentStyle={ttStyle} formatter={(v, _, props) => [`${v} reps avg · ${props.payload.sessions} sessions`, ""]} />
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={durations} margin={{ top: 5, right: 16, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip contentStyle={ttStyle} formatter={(v, _, props) => [`${v} min`, props.payload.label]} labelFormatter={shortFmt} />
                    {durations.length > 1 && (
